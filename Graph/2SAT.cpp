...
class two_sat {
  public:
    ...
    void add_xor(int a,int b,bool val) { equal(a,b,!val); }
    void set(int a,bool val) { add(a<<1^val,a<<1^val^1); }
    void except(int a,int b,bool val1,bool val2) {
      add(a<<1^val1^1,b<<1^val2);
      add(b<<1^val2^1,a<<1^val1);
    }
    void equal(int a,int b,bool val) {
      add(a<<1,b<<1^val^1);
      add(b<<1,a<<1^val^1);
      add(a<<1^1,b<<1^val);
      add(b<<1^1,a<<1^val);
